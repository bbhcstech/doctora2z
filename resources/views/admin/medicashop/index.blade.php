        ...
        <table id="example1" class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>@if($medicashop->image)
                        <!-- Display the current image if available -->
                        <div>
                            <img src="{{ asset('/admin/uploads/medicashop/' . $medicashop->image) }}" alt="Current Image" class="img-fluid mb-2" style="max-width: 50px;">
                        </div>
                    @else
                        <!-- Display the default image if no image is available -->
                        <div>
                            <img src="{{ asset('/admin/assets/adminimg/medicashop.jpg') }}" alt="Default Image" class="img-fluid mb-2" style="max-width: 50px;">
                        </div>
                    @endif</td>
                    <td>{{ $medicashop->address_link }}</td>
                    <td>
                        <a href="{{ route('medicashop.edit', $medicashop) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('medicashop.destroy', $medicashop) }}" method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
