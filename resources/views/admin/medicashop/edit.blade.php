...
@section('content')
    ...
    <main id="main" class="main">
        ...
        <div class="card" style="background-color: #d8e0f1; border-radius: 10px;">
            <div class="card-body">
                <form action="{{ route('medicashop.update', $medicashop->id) }}" method="POST"
                    ...
                    <div class="container"
                        ...
                        <div class="col-md-10 mb-3">
                            ...
                            <input type="text" name="name" class="form-control"
                                value="{{ old('name', $medicashop->name) }}" required>
                        </div>
                        ...
                        <div class="col-md-10 mb-3">
                            ...
                            <input type="text" name="address_link" class="form-control"
                                value="{{ old('address_link', $medicashop->address_link) }}" required>
                        </div>
                                ...
                                <select class="form-control" name="country_id" id="country_id" required>
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}"
                                            {{ old('country_id', $medicashop->country_id) == $country->id ? 'selected' : '' }}>
                                            {{ $country->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select class="form-control" name="state_id" id="state_id" required>
                                    ...
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}"
                                            {{ old('state_id', $medicashop->state_id) == $state->id ? 'selected' : '' }}>
                                            {{ $state->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select name="district_id" id="district_id" class="form-select" required>
                                    ...
                                    @foreach ($districts as $district)
                                        <option value="{{ $district->id }}"
                                            {{ old('district_id', $medicashop->district_id) == $district->id ? 'selected' : '' }}>
                                ...
                                <select name="city_id" id="city_id" class="form-select" required>
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ old('city_id', $medicashop->city_id) == $city->id ? 'selected' : '' }}>
                                            {{ $city->name }}</option>
                                    @endforeach
                                </select>
                            </div>
