      ...
      <form action="{{ route('advertisement.update', $advertisement->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <select class="form-control" name="state_id" id="state_id" required>
            ...
            @foreach ($states as $state)
              <option value="{{ $state->id }}" {{ $advertisement->state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="mb-3">
          ...
          @if (!empty($advertisement->image))
            <div class="mb-2">
              <img src="{{ asset('admin/uploads/advertisement/' . $advertisement->image) }}" alt="Advertisement Image" style="max-width: 150px; height: auto;">
            </div>
          @endif
