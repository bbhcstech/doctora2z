...
@endif
    ...
    <div class="row g-2">
      <div class="col-md-4">     
       ...
       <div class="col-md-4"> 
        <div class="mb-2">
            <label for="phone_number" class="form-label">Contact Number One <span style="color: red;">*</span></label>
            <input type="text" class="form-control"   class="form-control @error('phone_number') is-invalid @enderror" id="phone_number" name="phone_number" value="{{ old('phone_number', $client->phone_number) }}" required
    ...
    <div class="mb-2">
        ...
        <select class="form-control @error('country') is-invalid @enderror" name="country_id" id="country_id" required
            ...
            @foreach ($countries as $country)
                <option value="{{ $country->id }}" {{ $country->id == old('country_id', $client->country_id) ? 'selected' : '' }}>
                    {{ $country->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-2">
        ...
        <select class="form-control @error('state') is-invalid @enderror" name="state_id" id="state_id" required
            ...
            @foreach ($states as $state)
                <option value="{{ $state->id }}" {{ old('state_id', $client->state_id) == $state->id ? 'selected' : '' }}>
                    {{ $state->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-2">
        ...
        <select name="district_id" id="district_id" class="form-select" required
            ...
            @foreach ($districts as $district)
                <option value="{{ $district->id }}" {{ old('district_id', $client->district_id) == $district->id ? 'selected' : '' }}>
                    {{ $district->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-4"> 
    <div class="mb-2">
        <label for="name" class="form-label">City/Town/Village Name <span style="color: red;">*</span></label>
        ...
        <div class="mb-2">
            ...
            <input type="text" class="form-control"  class="form-control @error('phone_number2') is-invalid @enderror" id="phone_number2" name="phone_number2" value="{{ old('phone_number2', $client->phone_number2) }}" >
        </div>
    ...
    <div class="col-md-4"> 
        <div class="mb-2">
            ...
            <input type="text" class="form-control" id="pincode" name="pincode" value="{{ old('pincode', $client->pincode) }}">
        </div>
        </div>
          <!-- Website -->
    <div class="col-md-4"> 
    ...
    <div class="col-md-4">
        ...
        <select class="form-control" name="category_id[]" id="category_id" multiple required>
            @foreach ($category as $cat)
                <option value="{{ $cat->id }}" 
                    @if (in_array($cat->id, explode(',', $client->category_id))) selected @endif>
                    {{ $cat->name }}
                </option>
            @endforeach
        </select>
        ...
        <div class="col-md-4">
            <div class="mb-2">
                ...
                <input type="text" class="form-control" id="logitude" name="logitude" value="{{ old('logitude', $client->logitude) }}">
            </div>
        </div>
       ...
       <div class="row">
            ...
            <div class="col-md-12 mb-3">
                ...
                <input type="text" class="form-control @error('tags') is-invalid @enderror" id="tags" name="tags" value="{{ $client->tags }}" placeholder="Enter tags separated by commas">
                @error('tags')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
  ...
  <div class="col-md-4">
    <div class="mb-2">
        ...
        <div id="image-upload-container">
            ...
            <button type="button" class="btn btn-danger btn-sm remove-section-btn" style="display:none;">
            ...
            @foreach($existingImages as $index => $image)
                <div class="image-upload-section" id="image-section-{{ $index + 1 }}">
                    <div class="image-preview-container mt-2">
                        <img src="{{ asset($image->path) }}" alt="Existing Image" class="preview-thumbnail" style="max-width: 100px; margin-right: 10px; margin-bottom: 10px;">
                        <button type="button" class="btn btn-danger btn-sm remove-existing-image" data-image-id="{{ $image->id }}" data-index="{{ $index + 1 }}">Remove</button>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeCropper(inputField, previewContainer, sectionId) {
        ...
        reader.onload = function (e) {
            ...
            buttonsContainer.classList.add("buttons-container", "mt-2", "d-flex", "justify-content-between");
    ...
    function cropImage(previewContainer, img, inputField, removeButton) {
        const croppedCanvas = cropper.getCroppedCanvas({ width: 1500, height: 500 });
        croppedCanvas.toBlob(function (blob) {
            const croppedImageUrl = URL.createObjectURL(blob);

            // Replace preview with cropped image
            previewContainer.innerHTML = `<img src="${croppedImageUrl}" alt="Cropped Image" style="max-width: 100%; height: auto;">`;
    ...
    function createImageUploadSection() {
        ...
        fileInput.addEventListener("change", function () {
            ...
            addMoreButton.style.display = "inline-block"; // Show "Add More" button after uploading the first image
        });
    }
