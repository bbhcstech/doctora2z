            ...
            <div class="card-body py-3 px-3 px-md-4">
                <div class="row g-2 align-items-end">
                    <div class="col-12 col-md-3">
                        ...
                        <input id="pin_input" type="tel" class="form-control form-control-compact only-pin"
                            placeholder="Ex:- 712409" inputmode="numeric" maxlength="6" autocomplete="off">
                    </div>
                    ...
                    <div class="col-12 col-md-4 d-flex gap-2 justify-content-end">
                        <a href="{{ route('district.export.excel') }}" class="btn btn-outline-secondary btn-compact"
                            title="Export Excel">
                            <i class="bi bi-file-earmark-excel"></i>
                        </a>
                        <a href="{{ route('district.export.csv') }}" class="btn btn-outline-secondary btn-compact"
                            title="Export CSV">
                            <i class="bi bi-filetype-csv"></i>
                        </a>
                        <a href="{{ route('district.export.pdf') }}" class="btn btn-outline-secondary btn-compact"
                            title="Export PDF">
    ...
    <script>
        (function() {
            ...
            const URLS = {
                ...
                getStates: '{{ route('district.ajax.getStates') }}', // ?country_id=
                getDists: '{{ route('district.ajax.getDistricts') }}', // ?state_id=
                getCities: '{{ route('district.ajax.getCities') }}', // ?district_id=
            };
         ...
         <div class="toast-body ${cls}">${msg}</div></div>`
            ...
            $(document).on('input', '.only-pin', function() {
                const v = sanitizePin(this.value);
            ...
            $(document).ready(function() {
                if ($.fn.select2) $('#city_select').select2({
                    width: '100%',
            ...
            async function fillStates(countryId) {
                ...
                $('#city_select').prop('disabled', true).html('<option value="">Select city</option>').val(null)
                    .trigger('change');
                if (!countryId) return;
        ...
        <td class="text-nowrap">
          ...
          <button class="btn btn-sm btn-outline-danger delete-row">Delete</button>
            ...
            $('#addBtn').on('click', async function() {
                ...
                const r = await safeJsonFetch(URLS.store, {
                    method: 'POST',
                    body: fd
                });
                if (r.ok && r.json && r.json.success) {
            ...
            $(document).on('click', '.delete-row', async function() {
                ...
                const r = await safeJsonFetch(`${URLS.destroyBase}/${id}`, {
                    method: 'DELETE'
                });
                if (r.ok && r.json && r.json.success) {
                    $tr.remove();
