...
@section('content')
            ...
            <div class="card-body py-3 px-3 px-md-4">
                <form id="quickAddForm" class="row g-2 align-items-end">
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input id="qa_type" name="type" type="text" class="form-control form-control-compact"
                            placeholder="clinic / doctor / etc" required>
                    </div>
                    <div class="col-12 col-md-4">
                        <label class="form-label form-label-xs mb-1">Name</label>
                        <input id="qa_name" name="name" type="text" class="form-control form-control-compact"
                            placeholder="Category name" required>
                    </div>
                    <div class="col-12 col-md-3">
                        <label class="form-label form-label-xs mb-1">Image (optional)</label>
                        <input id="qa_image" name="image" type="file" accept="image/*"
                            class="form-control form-control-compact">
                    </div>
                    <div class="col-12 col-md-2 d-grid">
                        <button id="addBtn" type="button" class="btn btn-brand"><i
                                class="bi bi-plus-square me-1"></i>Add</button>
                    </div>
                </form>

                <div class="d-flex flex-column flex-md-row gap-2 justify-content-between mt-3">
                    <div class="btn-group">
                        <a href="{{ route('categories.sample') }}" class="btn btn-outline-secondary btn-compact"
                            data-bs-toggle="tooltip" title="Download Sample CSV">
                            <i class="bi bi-filetype-csv me-1"></i> Sample
                        </a>
                        <label class="btn btn-outline-secondary btn-compact mb-0" for="excel_file" data-bs-toggle="tooltip"
                    ...
                    <div class="btn-group">
                        <a href="{{ route('categories.export.csv') }}" class="btn btn-outline-secondary btn-compact"
                            data-bs-toggle="tooltip" title="Export CSV">
                            <i class="bi bi-filetype-csv me-1"></i> CSV
                        </a>
                        <a href="{{ route('categories.export.excel') }}" class="btn btn-outline-secondary btn-compact"
                            data-bs-toggle="tooltip" title="Export Excel">
                            <i class="bi bi-filetype-xlsx me-1"></i> Excel
                        </a>
                        <a href="{{ route('categories.export.pdf') }}" class="btn btn-outline-secondary btn-compact"
            ...
            <div class="card-body p-0">
                <div class="table-controls d-flex justify-content-between align-items-center p-3">
                    ...
                    <div>
                        ...
                        <input id="dt_search" type="search" class="form-control form-control-compact"
    ...
    <style>
        ...
        .btn-outline-success {
            ...
            --bs-btn-hover-bg: #16a34a10;
        }

        .btn-outline-danger {
            --bs-btn-color: #dc3545;
            --bs-btn-border-color: #dc3545;
            --bs-btn-hover-bg: #dc354510;
        }
    ...
    <script>
        (function() {
            ...
            function showToast(message, {
                ...
                const header = title ?
                    `<div class="toast-header border-0"><strong class="me-auto">${title}</strong><button type="button" class="btn-close" data-bs-dismiss="toast"></button></div>` :
                    '';
                const bodyClass = title ? '' : (type === 'success' ? 'bg-success text-white' : type === 'danger' ?
                    'bg-danger text-white' : 'bg-info text-dark');
                const html =
                    `<div id="${id}" class="toast shadow-sm mb-2" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="${delay}">${header}<div class="toast-body ${bodyClass}">${message}</div></div>`;
            ...
            let table = $('#categoriesTable').DataTable({
                ...
                createdRow: function(row, data) {
                    $(row).attr('data-id', data.id).attr('id', 'row-' + data.id);
                },
                columns: [{
                        data: null,
                        orderable: false,
                        ...
                        render: (_, __, row) =>
                            `<input type="checkbox" class="categoryCheckbox" data-id="${row.id}">`
                    },
                    {
                        data: 'id',
                        name: 'id'
                    },
                    {
                        ...
                        name: 'type',
                        render: (d) =>
                            `<span contenteditable="true" class="editable" data-field="type">${d??''}</span>`
                    },
                    {
                        data: 'name',
                        name: 'name',
                        render: (d) =>
                            `<span contenteditable="true" class="editable" data-field="name">${d??''}</span>`
                    },
                    {
                        data: 'image',
                        orderable: false,
          ...
          <button type="button" class="btn btn-square btn-outline-danger individualDeleteBtn ms-2" data-id="${row.id}" title="Delete Row">
            ...
            $('#dt_length').on('change', function() {
                table.page.len(Number(this.value)).draw(false);
            });
            let sTimer = null;
            $('#dt_search').on('input', function() {
                clearTimeout(sTimer);
                sTimer = setTimeout(() => {
            ...
            $('#categoriesTable').on('input', '.editable', function() {
            ...
            $(document).on('change', '.upload-input', function() {
                ...
                $btn.prop('disabled', true).html(
                    '<span class="upload-spinner" aria-hidden="true"></span> Uploading');
                ...
                $.ajax({
                    ...
                    if (res.success && (res.filename || res.url)) {
                        const newUrl = res.url ?? ('{{ asset('admin/uploads/category') }}/' + res
                            .filename);
            ...
            $('#categoriesTable').on('click', '.individualDeleteBtn', function() {
                ...
                confirmAction('Delete this category?', {
                    ...
                    if (!ok) return;
                    $.ajax({
                        url: '{{ route('categories.destroy', ['id' => ':id']) }}'.replace(
                            ':id', id),
                        method: 'DELETE'
                    }).done(res => {
                        if (res.success) {
            ...
            $('#bulkDeleteBtn').off('click').on('click', function() {
                ...
                confirmAction(`Delete ${ids.length} selected item(s)?`, {
                    okText: 'Delete',
                    okClass: 'btn-danger'
                }).then(ok => {
                    if (!ok) return;
                    ...
                    const oldHtml = $btn.html();
                    $btn.prop('disabled', true).html(
                        '<span class="upload-spinner" aria-hidden="true"></span> Deleting');
            ...
            $('#saveAllBtn').off('click').on('click', function() {
                ...
                const proceedToSave = () => {
                    if (!visibleEdited.length) {
                        notify.info(
                            'No visible edits to save on this page. Navigate to the edited rows to save them.'
                            );
                        return;
                    }
                    ...
                    const saveNext = () => {
                        ...
                        $.ajax({
                                url: '{{ route('categories.update', ['id' => ':id']) }}'.replace(
                                    ':id', id),
                                method: 'POST',
                                data: fd,
                                processData: false,
            ...
            $('#addBtn').on('click', function() {
                ...
                $.ajax({
                    ...
                    .done(res => {
                        if (res.success) {
                            notify.ok(res.message || 'Category created');
                            $('#quickAddForm')[0].reset();
                            table.ajax.reload(null, false);
                        } else notify.err(res.message || 'Failed to create category');
                    })
            ...
            document.getElementById('excel_file')?.addEventListener('change', function() {
                ...
                $.ajax({
                    ...
                    .done(res => {
                        res.success ? notify.ok(res.message || 'Import complete') : notify.err(res
                            .message || 'Import failed');
                        setTimeout(() => table.ajax.reload(null, false), 400);
                    })
