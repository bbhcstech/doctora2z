    ...
    <form id="bulkDeleteForm" action="{{ route('category.bulkDelete') }}" method="POST">
      ...
      <table id="example1" class="table table-striped table-bordered" style="font-size:13px;">
        ...
        <tbody>
          ...
          <tr>
            <td><input type="checkbox" class="categoryCheckbox" name="category_ids[]" value="{{ $category->id }}"></td>
            <td>{{ $index + 1 }}</td>
            <td>{{ $category->type }}</td>
            <td>{{ $category->name }}</td>
            <td>
              @if (!empty($category->image))
                <img src="{{ asset('admin/uploads/category/' . $category->image) }}" alt="Category Image" style="max-width: 100px; height: auto;">
              @else
                <span>No Image</span>
              @endif
            </td>
