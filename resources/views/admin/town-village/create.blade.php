        ...
        $j('#country_id').on('change', function() {
            ...
            //console.log('Selected Country ID:', countryId);
             ...
             if (countryId) {
                ...
                $j('#state_id').empty().append('<option value="">Select State</option>'); // Reset the state dropdown
            }
        });
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response && response.length > 0) {
                            ...
                            districtSelect.append('<option value="">No states found</option>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('AJAX Error:', error);
                        console.error('Full Response:', xhr.responseText);
                        alert('An error occurred while fetching district. Please try again.');
                    }
                });
            } else {
                alert('Please select a country.');
                $j('#district_id').empty().append('<option value="">Select District</option>'); // Reset the state dropdown
            }
        });
    });
</script>
