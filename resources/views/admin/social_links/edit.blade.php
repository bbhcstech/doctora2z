...
<main id="main" class="main">
  ...
  <form action="{{ route('social_links.update', $socialLink->id) }}" method="POST">
    ...
    <div class="mb-3">
      ...
      <input type="url" class="form-control" id="link_address" name="link_address" value="{{ $socialLink->link_address }}" required>
    </div>

    <div class="mb-3">
      <label for="link_icon" class="form-label">Link Icon (FontAwesome Class)</label>
      <input type="text" class="form-control" id="link_icon" name="link_icon" value="{{ $socialLink->link_icon }}" required>
    </div>
    </div>
    </div>
    <div class="text-end">
