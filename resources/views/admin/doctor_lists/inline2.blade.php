                        ...
                        <tbody>
                            @foreach ($doctors as $index => $doctor)
                                <tr data-id="{{ $doctor->id }}">
                                    ...
                                    <td data-field="category_id" data-category-id="{{ $doctor->category_id }}">
                                        {{ $doctor->category->name ?? 'N/A' }}</td>
                                    <td data-field="clinic_id" data-clinic-id="{{ $doctor->clinic_id }}">
                                        {{ $doctor->client->name ?? 'N/A' }}</td>
                                    <td data-field="country_id" data-country-id="{{ $doctor->country_id }}">
                                        {{ $doctor->country->name ?? 'N/A' }}</td>
                                    <td data-field="state_id" data-state-id="{{ $doctor->state_id }}">
                                        {{ $doctor->state->name ?? 'N/A' }}</td>
                                    <td data-field="district_id" data-district-id="{{ $doctor->district_id }}">
                                        {{ $doctor->district->name ?? 'N/A' }}</td>
                                    <td data-field="city_id" data-city-id="{{ $doctor->city_id }}">
                                        {{ $doctor->city->name ?? 'N/A' }}</td>
                                    <td data-field="status">
                                        <span
                                            class="badge bg-{{ $doctor->status == 'active' ? 'success' : 'secondary' }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#state_id').on('change', function() {
                ...
                if (state_id) {
                    $.ajax({
                        url: "{{ route('doctor.getDistrictsByState', ':id') }}".replace(':id',
                            state_id),
                        type: "GET",
                        success: function(response) {
                            if (response.success) {
            ...
            $('#district_id').on('change', function() {
                ...
                if (district_id) {
                    $.ajax({
                        url: "{{ route('doctor.getCitiesByDistrict', ':id') }}".replace(':id',
                            district_id),
                        type: "GET",
                        success: function(response) {
                            if (response.success) {
            ...
            $('#reset-form').click(function() {
                ...
                $('#state_id, #district_id, #city_id').html('<option value="">Select...</option>').prop(
                    'disabled', true);
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (doctor.country_id) {
                                ...
                                setTimeout(() => {
                                    if (doctor.state_id) {
                                        $('#state_id').val(doctor.state_id).trigger(
                                            'change');
                                        setTimeout(() => {
                                            if (doctor.district_id) {
                                                $('#district_id').val(doctor
                                                    .district_id).trigger(
                                                    'change');
                                                setTimeout(() => {
                                                    if (doctor
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#doctor-id').val('');
                            $('#form-title').text('Add New Doctor');
                            $('#state_id, #district_id, #city_id').html(
                                '<option value="">Select...</option>').prop('disabled',
                                true);
                            $('#country_id').val('').trigger('change');
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    if (deletedCount === totalCount) {
                                        table.draw();
                                        showAlert('success',
                                            `${totalCount} doctor(s) deleted successfully.`
                                            );
            ...
            function showAlert(type, message) {
                ...
                alert.removeClass('d-none alert-success alert-danger alert-warning alert-info')
                    .addClass(`alert-${type}`)
                    .html(
                        `<i class="bi ${type === 'success' ? 'bi-check-circle' : type === 'danger' ? 'bi-exclamation-octagon' : 'bi-exclamation-triangle'} me-2"></i>${message}`
                        );
