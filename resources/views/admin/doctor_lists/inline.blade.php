    ...
    <form id="bulkForm" method="POST" action="{{ route('doctors.bulk-store') }}">
        ...
        <div class="mb-3">
            @if (auth()->user()->role == 'admin')
                ...
                <button type="button" class="btn btn-danger" id="bulkDelete" formaction="{{ route('doctors.bulk-delete') }}">Delete Selected</button>
            @endif
        </div>
                ...
                <tbody id="doctorTableBody">
                    @foreach ($doctors as $index => $doctor)
                        ...
                        <tr data-id="{{ $doctor->id }}">
                            <td><input type="checkbox" name="selected_ids[]" value="{{ $doctor->id }}" class="selectRow"></td>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $doctor->id }}</td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][name]" value="{{ $doctor->name }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][phone_number]" value="{{ $doctor->phone_number }}" class="form-control"></td>
                            <td><input type="email" name="doctors[{{ $doctor->id }}][email]" value="{{ $user->email ?? '' }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][degree]" value="{{ $doctor->degree }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][reg_no]" value="{{ $doctor->reg_no }}" class="form-control"></td>
                            <td>
                                <select name="doctors[{{ $doctor->id }}][category_id][]" multiple class="form-control">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{ in_array($category->id, $categoryIds) ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][sub_category]" value="{{ $doctor->sub_category }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][personal_phone_number]" value="{{ $doctor->personal_phone_number }}" class="form-control"></td>
                            <td>
                                <select name="doctors[{{ $doctor->id }}][month][]" multiple class="form-control">
                                    @foreach (['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $month)
                                        <option value="{{ $month }}" {{ in_array($month, $months) ? 'selected' : '' }}>{{ $month }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                <select name="doctors[{{ $doctor->id }}][day][]" multiple class="form-control">
                                    @foreach (['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                        <option value="{{ $day }}" {{ in_array($day, $days) ? 'selected' : '' }}>{{ $day }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                @foreach ($timeSlots as $slotIndex => $slot)
                                    <div class="input-group mb-1">
                                        <input type="time" name="doctors[{{ $doctor->id }}][time_slot][{{ $slotIndex }}][start]" value="{{ explode(' - ', $slot)[0] ?? '' }}" class="form-control">
                                        <input type="time" name="doctors[{{ $doctor->id }}][time_slot][{{ $slotIndex }}][end]" value="{{ explode(' - ', $slot)[1] ?? '' }}" class="form-control">
                                    </div>
                                @endforeach
                                <button type="button" class="btn btn-sm btn-outline-secondary add-time-slot">Add Time Slot</button>
                            </td>
                            <td><textarea name="doctors[{{ $doctor->id }}][profile_text]" class="form-control">{{ $doctor->profile_text }}</textarea></td>
                            <td>
                                @if ($doctor->image)
                                    <img src="{{ asset('/admin/uploads/doctor/' . $doctor->image) }}" alt="Doctor Image" class="img-fluid mb-2" style="max-width: 50px;">
                                @else
                            ...
                            <td><input type="number" name="doctors[{{ $doctor->id }}][fees]" value="{{ $doctor->fees }}" class="form-control" step="0.01"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][whatsapp]" value="{{ $doctor->whatsapp }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][facebook]" value="{{ $doctor->facebook }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][instagram]" value="{{ $doctor->instagram }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][website]" value="{{ $doctor->website }}" class="form-control"></td>
                            <td><input type="number" name="doctors[{{ $doctor->id }}][latitude]" value="{{ $doctor->latitude }}" class="form-control" step="0.000001"></td>
                            <td><input type="number" name="doctors[{{ $doctor->id }}][logitude]" value="{{ $doctor->logitude }}" class="form-control" step="0.000001"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][language]" value="{{ $doctor->language }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][mode_of_payment]" value="{{ $doctor->mode_of_payment }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][loc1]" value="{{ $doctor->loc1 }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][loc2]" value="{{ $doctor->loc2 }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][loc3]" value="{{ $doctor->loc3 }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][loc4]" value="{{ $doctor->loc4 }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][loc5]" value="{{ $doctor->loc5 }}" class="form-control"></td>
                            <td><input type="text" name="doctors[{{ $doctor->id }}][membership]" value="{{ $doctor->membership }}" class="form-control"></td>
                            <td>
                                <select name="doctors[{{ $doctor->id }}][clinic_ids]" class="form-control">
                                    <option value="">Select Clinic</option>
                                    @foreach ($clinics as $clinic)
                                        <option value="{{ $clinic->id }}" {{ in_array($clinic->id, $clinicIds) ? 'selected' : '' }}>{{ $clinic->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                <select name="doctors[{{ $doctor->id }}][state_id]" class="form-control">
                                    ...
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}" {{ $doctor->state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="checkbox" name="doctors[{{ $doctor->id }}][active]" value="1" {{ $doctor->active ? 'checked' : '' }}>
                            </td>
                            <td>{{ $doctor->status ?? 'N/A' }}</td>
                            <td>{{ $doctor->updated_at ? \Carbon\Carbon::parse($doctor->updated_at)->format('d-m-Y') : 'N/A' }}</td>
                            @if (auth()->user()->role == 'admin')
                                <td>
                                    <button type="button" class="btn btn-success btn-sm save-row" data-id="{{ $doctor->id }}">Save</button>
                                    <button type="button" class="btn btn-danger btn-sm delete-row" data-id="{{ $doctor->id }}">Delete</button>
                                    <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#statusModal" data-id="{{ $doctor->id }}" data-status="{{ $doctor->status }}">Change Status</button>
                                </td>
                            @endif
                        </tr>
                    @endforeach
    ...
    $jq(document).ready(function() {
        ...
        $jq(document).on('click', '.delete-row', function() {
            ...
            if (confirm('Are you sure you want to delete this doctor?')) {
                $jq.ajax({
                    url: '{{ route("doctors.ajax-destroy", ["id" => ":id"]) }}'.replace(':id', id),
                    method: 'DELETE',
                    success: function(response) {
                        if (response.success) {
                            table.row(row).remove().draw();
        ...
        $jq('#bulkDelete').on('click', function() {
            ...
            if (confirm('Are you sure you want to delete the selected doctors?')) {
                $jq.ajax({
                    ...
                    method: 'POST',
                    data: { selected_ids: selectedIds, _method: 'POST' },
                    success: function(response) {
                        if (response.success) {
                            alert(response.message);
                            location.reload();
