        ...
        <div class="col-md-4 mb-3">
          ...
          <div class="row">
            ...
            <div class="mb-2">
                ...
                <select class="form-control @error('country') is-invalid @enderror" name="country_id" id="country_id" required
                    oninvalid="this.setCustomValidity('Please select country')" 
                    oninput="this.setCustomValidity('')">
                    <option value="">Select a country</option>
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" data-name="{{ $country->name }}" {{ old('country_id', $doctor->country_id) == $country->id ? 'selected' : '' }}>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-2">
                ...
                <select class="form-control @error('state') is-invalid @enderror" name="state_id" id="state_id" required
...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        <select class="form-control" name="category_id[]" id="category_id" multiple required>
            @foreach ($category as $cat)
                <option value="{{ $cat->id }}" 
                    @if (in_array($cat->id, explode(',', $doctor->category_id))) selected @endif>
                    {{ $cat->name }}
                </option>
            @endforeach
        </select>
        ...
        <label for="reg_no" class="form-label">Registration No </label>
                ...
                <input type="text" class="form-control @error('reg_no') is-invalid @enderror" id="reg_no"  name="reg_no" value="{{ old('reg_no', $doctor->reg_no) }}"  >
        ...
        <div class="col-md-4 mb-3">
          ...
          <input type="text" class="form-control @error('degree') is-invalid @enderror" id="degree" name="degree" value="{{ old('degree', $doctor->degree) }}">
          @error('degree')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
    ...
    <div class="col-md-4 mb-3">
          ...
          <input type="text" pattern="\d{10}" maxlength="10" minlength="10" class="form-control @error('personal_phone_number') is-invalid @enderror" id="personal_phone_number" name="personal_phone_number" value="{{ old('personal_phone_number', $doctor->personal_phone_number) }}">
        ...
        <div class="col-md-4 mb-3">
          ...
          <input type="text"  class="form-control @error('visiting_time') is-invalid @enderror" id="visiting_time" name="visiting_time" value="{{ old('visiting_time', $doctor->visiting_time) }}">
          @error('visiting_time')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
...
<div class="col-md-4 mb-3" id="date-picker-container" style="display: {{ old('date_wise_checkbox', $doctor->date_wise_checkbox) ? 'block' : 'none' }};">
    <label for="date_picker">Select Date:</label>
    <input type="text" id="date_picker" name="date_picker" class="form-control" value="{{ old('date_picker', $doctor->date_picker) }}" />
    @error('date_picker')
...
<div class="col-md-4 mb-3" id="month-container" style="display: {{ old('day_wise_checkbox', $doctor->day_wise_checkbox) ? 'block' : 'none' }};">
    ...
    <select id="month" name="month[]" class="form-control w-100" multiple>
        ...
        @foreach ($months as $monthValue => $monthName)
            <option value="{{ $monthValue }}" {{ in_array($monthValue, $selectedMonths) ? 'selected' : '' }}>{{ $monthName }}</option>
        @endforeach
    </select>
    @error('month')
        <div class="text-danger">{{ $message }}</div>
...
<!-- Day Select (Day Wise) -->
<div class="col-md-4 mb-3" id="day-container" style="display: {{ old('day_wise_checkbox', $doctor->day_wise_checkbox) ? 'block' : 'none' }};">
    ...
    <select id="day" name="day[]" class="form-control w-100" multiple>
        ...
        <option value="everyday" {{ in_array('everyday', $selectedDays) ? 'selected' : '' }}>Everyday</option>

        @foreach ($days as $dayValue => $dayName)
            <option value="{{ $dayValue }}" {{ in_array($dayValue, $selectedDays) ? 'selected' : '' }}>{{ $dayName }}</option>
        @endforeach
    </select>
    @error('day')
        <div class="text-danger">{{ $message }}</div>
...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="checkbox" id="time_checkbox" name="time_checkbox" value="1" {{ old('time_checkbox', $doctor->time_checkbox) == 1 ? 'checked' : '' }}>
    </div>

    <!-- Time Fields Container -->
    <div class="col-md-12 mb-3" id="time-fields-container" style="display: {{ old('time_checkbox', $doctor->time_checkbox) ? 'block' : 'none' }};">
        <div id="time-fields-wrapper">
            ...
            @php
                ...
                $timeSlots = old('time_slot', $doctor->time_slot) ? explode(', ', $doctor->time_slot) : [];
            @endphp
                ...
                <div class="time-field-pair">
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="time_slot[{{ $index }}][start]" class="form-control start-time" 
                                   value="{{ $times[0] ?? '' }}" required>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="time_slot[{{ $index }}][end]" class="form-control end-time" 
                                   value="{{ $times[1] ?? '' }}" required>
...
<div class="row">
  ...
  <div class="row">
            ...
            <div class="col-md-12 mb-3">
                ...
                <input type="text" class="form-control @error('tags') is-invalid @enderror" id="tags" name="tags" value="{{ $doctor->tags}}" placeholder="Enter tags separated by commas">
                @error('tags')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
  ...
  <div class="row">
    ...
    <div class="form-group">
        ...
        <input type="date" class="form-control" name="last_update" id="last_update" value="{{ \Carbon\Carbon::parse($doctor->last_update)->format('Y-m-d') }}">
    </div>
    </div>
    ...
    <div class="col-md-4 mb-3">
        ...
        @if($doctor->image)
            <div>
                <img src="{{ asset('/admin/uploads/doctor/' . $doctor->image) }}" alt="Current Image" class="img-fluid mb-2" style="max-width: 100px;">
                <p>Current Image</p>
            </div>
        @else
            <div>
                <img src="{{ asset('/admin/assets/adminimg/demo_doctor_image.jpeg') }}" alt="Default Image" class="img-fluid mb-2" style="max-width: 100px;">
                <p>Default Image</p>
            </div>
        @endif
           ...
           <!-- Fess -->
            <div class="col-md-4 mb-3">
                ...
                <input type="number" class="form-control @error('fees') is-invalid @enderror"
                       id="fees" name="fees" value="{{ $doctor->fees}}" 
...
<div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('website') is-invalid @enderror"
                       id="website" name="website" value="{{ old('website', $doctor->website) }}">
                @error('website')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('mode_of_payment') is-invalid @enderror"
                       id="mode_of_payment" name="mode_of_payment" value="{{ old('mode_of_payment', $doctor->mode_of_payment) }}">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('loc1') is-invalid @enderror"
                       id="loc1" name="loc1" value="{{ old('loc1', $doctor->loc1) }}">
                @error('loc1')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('loc2') is-invalid @enderror"
                       id="loc2" name="loc2" value="{{ old('loc2', $doctor->loc2) }}">
                @error('loc2')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('loc3') is-invalid @enderror"
                       id="loc3" name="loc3" value="{{ old('loc3', $doctor->loc3) }}">
                @error('loc3')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('loc4') is-invalid @enderror"
                       id="loc4" name="loc4" value="{{ old('loc4', $doctor->loc4) }}">
                @error('loc4')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" class="form-control @error('loc5') is-invalid @enderror"
                       id="loc5" name="loc5" value="{{ old('loc5', $doctor->loc5) }}">
                @error('loc5')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                success: function(response) {
                    ...
                    if (response.length > 0) {
                        $.each(response, function(index, state) {
                            ...
                            stateSelect.append(`<option value="${state.id}" data-name="${state.name}" ${isSelected}>${state.name}</option>`);
                        });
                    ...
                    success: function(response) {
                        ...
                        if (response.length > 0) {
                            $.each(response, function(index, district) {
                                let isSelected = district.id == selectedDistrict ? 'selected' : '';
                                districtSelect.append(`<option value="${district.id}" data-name="${district.name}" ${isSelected}>${district.name}</option>`);
                            });
