      ...
      <form action="{{ route('hospital.update', $hospital->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="container" style="background-color: #d8e0f1; padding: 20px; border-radius: 10px; margin:20px;">
          ...
          <div class="col-md-10 mb-3">
              ...
              <input type="text" name="address_link" class="form-control" value="{{ old('address_link', $hospital->address_link) }}" required>
          </div>
              ...
              <div class="mb-3">
                  ...
                  <select class="form-control" name="state_id" id="state_id" required>
                      ...
                      @foreach ($states as $state)
                          <option value="{{ $state->id }}" {{ old('state_id', $hospital->state_id) == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                      @endforeach
                  </select>
              </div>
          </div>
              ...
              <div class="mb-3">
                  ...
                  <select name="city_id" id="city_id" class="form-select" required>
                      ...
                      @foreach ($cities as $city)
                          <option value="{{ $city->id }}" {{ old('city_id', $hospital->city_id) == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                      @endforeach
                  </select>
              </div>
          </div>
        ...
        @if($hospital->image)
            <div>
                <img src="{{ asset('/admin/uploads/hospitals/' . $hospital->image) }}" alt="Current Image" class="img-fluid mb-2" style="max-width: 100px;">
                <p>Current Image</p>
            </div>
        @else
            <div>
                <img src="{{ asset('/admin/assets/adminimg/hospital.jpg') }}" alt="Default Image" class="img-fluid mb-2" style="max-width: 100px;">
                <p>Default Image</p>
            </div>
        @endif
