...
<form action="{{ route('doctor.profile.updateEducationSchedule', $d->id ?? '') }}" method="POST" id="educationFormPartial">
  @csrf
      ...
      <div class="d-flex gap-2 flex-wrap">
        ...
        @foreach($daysMap as $token => $label)
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="checkbox" name="clinic_days[]" id="edu_day_{{ $token }}" value="{{ $token }}" @checked(in_array($token, $initialClinicDays))>
            <label class="form-check-label small" for="edu_day_{{ $token }}">{{ $label }}</label>
          </div>
        @endforeach

        <div class="ms-3">
          <label class="form-label small mb-1 d-block">Start</label>
          <input type="time" name="clinic_start_time" class="form-control form-control-sm" value="{{ old('clinic_start_time', isset($d->clinic_start_time) ? \Carbon\Carbon::parse($d->clinic_start_time)->format('H:i') : '') }}">
        </div>
        <div class="ms-2">
          <label class="form-label small mb-1 d-block">End</label>
          <input type="time" name="clinic_end_time" class="form-control form-control-sm" value="{{ old('clinic_end_time', isset($d->clinic_end_time) ? \Carbon\Carbon::parse($d->clinic_end_time)->format('H:i') : '') }}">
        </div>
      </div>
    </div>
