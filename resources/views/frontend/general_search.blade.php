    ...
    @foreach ($countries->sortBy('name') as $country)
        ...
        @if ($firstLetter !== $currentLetter)
            ...
            <div class="mt-3">
                ...
                <h5 class="fw-bold text-white px-3 py-1 rounded" style="background-color: #0044cc; display: inline-block;">{{ $firstLetter }}</h5>
                <div class="row">
        ...
        <div class="col-md-3 mb-2">
            <button 
                class="btn btn-outline-primary btn-sm country-btn w-100 text-start" 
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
                ...
                fetch(`/public/get-states/${countryId}`)
                    ...
                    .then(data => {
                        ...
                        data.forEach(state => {
                            ...
                            stateButton.className = 'btn btn-outline-secondary btn-sm';
        ...
        stateButtonsContainer.addEventListener('click', function (event) {
            ...
            if (target.tagName === 'BUTTON') {
                ...
                document.getElementById('districts-heading').textContent = `Districts/City/Town/Village in ${stateName}:`;
    ...
    data.forEach(doctor => {
         let ratingStars = "";
        let averageRating =  fetchDoctorRating(doctor.id, doctor.name); // Ensure rating exists
        for (let i = 1; i <= 5; i++) {
            ratingStars += `<i class="fa fa-star ${i <= averageRating ? 'text-warning' : 'text-secondary'}"></i>`;
        }
        const doctorCard = `
        ...
        `;
        document.getElementById('doctor_details').innerHTML += doctorCard;
    });
} else {
    document.getElementById('doctor_details').innerHTML = '<p>No doctors available for this city.</p>';
}
                    })
                    .catch(error => {
...
<style>
    ...
    .country-item {
        width: 24%; /* 4 columns per row (100% / 4 = 25%, reduced slightly for spacing) */
        padding: 5px 0;
