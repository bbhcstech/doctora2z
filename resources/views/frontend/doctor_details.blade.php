...
<html lang="en">
    ...
    <head>
        ...
        <meta
            content="View detailed profile of Dr. {{ $doctor->name }}, {{ $doctor->category->name ?? 'Medical Professional' }} in {{ $doctor->city->name ?? '' }}, {{ $doctor->state->name ?? '' }}"
            name="description">
            ...
            <div class="doctor-details-container">
                ...
                <div class="profile-card">
                    ...
                    @php
                        ...
                        if (
                            !$primaryVisitingTime &&
                            $doctor->visiting_locations &&
                            $doctor->visiting_locations->isNotEmpty()
                        ) {
                    ...
                    <div class="doctor-header">
                        ...
                        <div class="avatar-section">
                            ...
                            <!-- image -->
                            <img id="doctorAvatar" src="{{ $doctorImage }}" alt="Dr. {{ $doctor->name }}"
                                class="doctor-avatar" onerror="handleImageError(this)">
                            ...
                            <div class="rating-section">
                                ...
                                <div class="rating-text">Rating: {{ number_format($doctor->avg_rating, 1) }}
                                    ({{ $doctor->rating_count }} reviews)</div>
                            </div>
                    ...
                    <div class="map-section section-spacing">
                        ...
                        <div class="map-container">
                            @if ($doctor->map_address)
                                <iframe
                                    src="https://maps.google.com/maps?q={{ urlencode($doctor->map_address) }}&t=&z=13&ie=UTF8&iwloc=&output=embed"
                                    width="100%" height="100%" style="border:0;" allowfullscreen=""
                                    loading="lazy"></iframe>
                            @else
                                <div
                                    style="display: flex; justify-content: center; align-items: center; height: 100%; background: #f8f9fa; color: #666;">
                                ...
                                @foreach ($relatedDoctors as $relatedDoctor)
                                    @php
                                        ...
                                        if ($relatedDoctor->profile_picture) {
                                            ...
                                            } elseif (str_starts_with($relatedDoctor->profile_picture, 'storage/')) {
                                                $relatedDoctorImage = asset($relatedDoctor->profile_picture);
                                            } elseif (
                                                file_exists(public_path('images/' . $relatedDoctor->profile_picture))
                                            ) {
                                                $relatedDoctorImage = asset(
                                                    'images/' . $relatedDoctor->profile_picture,
                                                );
                                            } else {
                                                $relatedDoctorImage = asset(
                                                    'storage/' . $relatedDoctor->profile_picture,
                                                );
                                            }
                                        }
                                    @endphp
                                    <div class="related-card" data-doctor-id="{{ $relatedDoctor->id }}">
                                        <img src="{{ $relatedDoctorImage }}" alt="Dr. {{ $relatedDoctor->name }}"
                                            class="related-avatar" onerror="this.src='{{ $defaultImage }}'">
                                        <h3 class="related-name">Dr. {{ $relatedDoctor->name }}</h3>
                                        <p class="related-specialty">
                                            {{ $relatedDoctor->category->name ?? 'Medical Professional' }}</p>
                                        ...
                                        <p class="rating-text">{{ number_format($relatedDoctor->avg_rating, 1) }}
                                            ({{ $relatedDoctor->rating_count }})</p>
                                        <a href="{{ route('doctor.details', $relatedDoctor->id) }}"
            ...
            <div class="popup-content">
                ...
                <form id="ratingForm" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="rating_user_email" name="user_email" class="form-control"
                            required>
                        <div id="rating_email_error"
                            style="color:#c00; font-size:13px; display:none; margin-top:6px;"></div>
                    </div>
                ...
                $('#ratingForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            if (res && res.success) {
                                if (typeof res.average !== 'undefined' && typeof res.count !==
                                    'undefined') {
                                ...
                                $('#rating_error').show().text(res.message ||
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr && xhr.status === 422 && xhr.responseJSON) {
                                ...
                                if (errors.rating || errors.rating_point) {
                                    $('#rating_error').show().text((errors.rating && errors.rating[
                                        0]) || (errors.rating_point && errors.rating_point[
                                        0]) || 'Invalid rating');
                                }
                                if (errors.user_email || errors.email) {
                                    $('#rating_email_error').show().text((errors.user_email &&
                                        errors.user_email[0]) || (errors.email && errors
                                        .email[0]) || 'Invalid email');
                                }
                            } else {
                                const msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr
                                    .responseJSON.message : 'Server error while saving rating';
