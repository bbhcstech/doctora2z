        ...
        @php
            ...
            if ($doctor && !empty($doctor->profile_picture)) {
                ...
                if (Str::startsWith($path, ['http://', 'https://'])) {
                    ...
                    $avatarUrl = asset('admin/uploads/doctor/' . $path);
                } elseif (file_exists(public_path('admin/uploads/doctor/' . ($doctor->id ?? '') . '/' . $path))) {
                    $avatarUrl = asset('admin/uploads/doctor/' . ($doctor->id ?? '') . '/' . $path);
                }
            }
                ...
                @if (!$doctor)
                    <div class="col-12">
                        <div class="alert alert-warning">
                            Profile not found. <a class="alert-link" href="{{ route('doctor.profile.edit') }}">Create your
                                profile</a>.
                        </div>
                    </div>
                @else
                                ...
                                <div class="profile-wrap">
                                    ...
                                    <div class="stack">
                                        ...
                                        <div class="chips">
                                            ...
                                            @else
                                                <span class="chip"
                                                    style="background:#fff7ed;color:#92400e;border-color:#fed7aa">
                                                    <i class="bi bi-exclamation-triangle"></i> Inactive
                                                </span>
                                            @endif
                                            <span class="chip"><i class="bi bi-people"></i> Mode:
                                                {{ safe_string(ucfirst($doctor->consultation_mode ?? '')) }}</span>
                                            <span class="chip"><i class="bi bi-tag"></i> Category:
                                                {{ safe_string(optional($doctor->category)->name ?? $doctor->category_id) }}</span>
                                        </div>
                                        <div class="small">
                                            <i class="bi bi-envelope me-1"
                                                style="color:#1d4ed8"></i>{{ safe_string($doctor->email ?? Auth::user()->email) }}
                                            &nbsp;&nbsp;
                                            <i class="bi bi-phone me-1"
                                                style="color:#059669"></i>{{ safe_string($doctor->phone_number) }}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="col-lg-7 col-12 stretch">
                        <div class="card-flat">
                            ...
                            <div class="section-title"
                                style="border:none; padding:0; margin: var(--space-3) 0 var(--space-2)">
                                <i class="bi bi-share"></i> Social & Links
                            </div>
                            <div class="social-list">
                                @if ($doctor->whatsapp)
                                    <a class="soc active whatsapp"
                                        href="https://wa.me/{{ preg_replace('/\D/', '', $doctor->whatsapp) }}"
                                        target="_blank" title="WhatsApp"><i class="bi bi-whatsapp"></i></a>
                                @else
                                    <span class="soc" title="WhatsApp not provided"><i class="bi bi-whatsapp"></i></span>
                                @endif

                                @if ($doctor->email ?? Auth::user()->email)
                                    <a class="soc active email"
                                        href="mailto:{{ e($doctor->email ?? Auth::user()->email) }}" title="Email"><i
                                            class="bi bi-envelope-fill"></i></a>
                                @else
                                    <span class="soc" title="Email not provided"><i class="bi bi-envelope"></i></span>
                                @endif

                                @if ($doctor->facebook)
                                    <a class="soc active facebook" href="{{ e($doctor->facebook) }}" target="_blank"
                                        title="Facebook"><i class="bi bi-facebook"></i></a>
                                @else
                                    <span class="soc" title="Facebook not provided"><i
                                            class="bi bi-facebook"></i></span>
                                @endif

                                @if ($doctor->instagram)
                                    <a class="soc active instagram" href="{{ e($doctor->instagram) }}" target="_blank"
                                        title="Instagram"><i class="bi bi-instagram"></i></a>
                                @else
                                    <span class="soc" title="Instagram not provided"><i
                                            class="bi bi-instagram"></i></span>
                                @endif

                                @if ($doctor->website)
                                    <a class="soc active website" href="{{ e($doctor->website) }}" target="_blank"
                                        title="Website"><i class="bi bi-globe2"></i></a>
                                @else
                                    <span class="soc" title="Website not provided"><i class="bi bi-globe2"></i></span>
                                @endif
                            </div>
                    ...
                    <div class="col-lg-5 col-12 stretch">
                        <div class="card-flat">
                            <div class="section-title"><i class="bi bi-clipboard-check"></i> Basic & Professional Info
                            </div>

                            <div class="kv">
                                <div class="label">Degrees</div>
                                <div class="value">
                                    {{ !empty($degreesArr) ? implode(', ', $degreesArr) : 'Not provided yet' }}</div>
                                ...
                                <div class="value">
                                    {{ safe_string(optional($doctor->category)->name ?? $doctor->category_id) }}</div>
                            </div>
                    ...
                    <div class="col-12">
                        <div class="card-flat">
                            ...
                            @if ($clinicSchedules->isEmpty())
                                <div class="muted">No schedules yet. <a class="link"
                                        href="{{ route('doctor.profile.edit') }}">Add a clinic schedule +</a></div>
                            @else
                                    ...
                                    <table class="table table-flat align-middle">
                                        ...
                                        <tbody>
                                            @foreach ($clinicSchedules as $idx => $s)
                                                @php
                                                    $daysStr = is_array($s->days)
                                                        ? implode(', ', $s->days)
                                                        : safe_string($s->days, '');
                                                    $daysArr = array_filter(
                                                        array_map('trim', explode(',', strtolower($daysStr))),
                                                    );
                                                ...
                                                <tr class="{{ $isToday ? 'today-row' : '' }}">
                                                    <td>{{ $idx + 1 }}</td>
                                                    <td class="fw-bold"><i
                                                            class="bi bi-hospital me-1"></i>{{ safe_string(optional($s->clinic)->name ?? $s->clinic_id) }}
                                                    </td>
                                                    <td>
